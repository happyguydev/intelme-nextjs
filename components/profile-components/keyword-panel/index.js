...
import passwordPanelStyles from './style';
...
const KeywordPanel = ({ userDetail, disciplines }) => {
  ...
  useEffect(() => {
    setKeywords(
      userDetail.settings.keywords.map((keyword) => {
        return { key: keyword };
      })
    );
  ...
  const handleDeleteKeyword = async () => {
    ...
    try {
      ...
      const userData = {
        jobTitle: userDetail.settings.jobTitle,
        mobileNo: userDetail.settings.mobileNo,
    ...
    if (addedChips.length > 0) {
      try {
        ...
        api.getUserProfile({ username }).then((res) => {
          ...
          toast.success(
            'You have successfully added your password!',
            toastConfig
          );
